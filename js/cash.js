const cashModule = {
    ...
    init() {
        ...
        this.checkDayChange();
        // ✅ PERBAIKAN: Disable auto repair yang bisa menyebabkan duplikat
    ...
    // ✅ PERBAIKAN: Hapus penambahan modal ganda di checkDayChange
    checkDayChange() {
            ...
            if (!isKasirOpen) {
                ...
                if (modalAwal > 0) {
                    ...
                    if (confirm(`🌅 Selamat datang di hari baru!\n\nKas kemarin: Rp ${utils.formatNumber(kasKemarin)}\nModal awal kemarin: Rp ${utils.formatNumber(modalAwal)}\n\nKasir dalam keadaan TUTUP.\n\nSetel kas ke Rp ${utils.formatNumber(modalAwal)} (Modal Awal) atau Rp 0?`)) {
                        ...
                        // ✅ PERBAIKAN: Hanya set currentCash, JANGAN tambah transaksi lagi
                        // Karena modal sudah tercatat saat user input modal awal
                    ...
                    if (newModal > 0) {
                        // ✅ PERBAIKAN: Hanya set currentCash, transaksi modal sudah di-handle di saveModalAwal
    ...
    // ✅ PERBAIKAN: Fungsi repair di-disable sementara atau diubah logic
    repairMissingModalTransactions() {
        // Cek apakah sudah pernah repair hari ini
        const lastRepair = localStorage.getItem('hifzi_last_repair');
        const today = new Date().toDateString();
        
        if (lastRepair === today) {
            return false; // Sudah repair hari ini, skip
        }
        ...
        // ✅ PERBAIKAN: Hanya repair jika selisih > 0 dan bukan dari modal yang belum ke-record
        // dengan memeriksa apakah modal sudah ada di transaksi hari ini
        if (selisih > 0) {
            ...
            // ✅ PERBAIKAN: Jika selisih sama dengan currentCash, berarti ini modal awal yang belum tercatat
            // tapi jangan auto-repair, biarkan user yang input manual lewat tombol Modal Awal
            if (!hasModalToday && selisih !== currentCash) {
                ...
                dataManager.save();
                localStorage.setItem('hifzi_last_repair', today);
    ...
    // ✅ PERBAIKAN: Tombol manual repair di UI - lebih aman
    openRepairModal() {
        const currentCash = parseInt(dataManager.data.settings?.currentCash) || 0;
        const calculatedCash = this.calculateCashFromTransactions();
        const selisih = currentCash - calculatedCash;
        
        // ✅ PERBAIKAN: Cek apakah selisih karena modal belum tercatat
        const today = new Date();
        today.setHours(0,0,0,0);
        ...
        const isModalIssue = selisih > 0 && !hasModalToday;
                    ...
                    <div style="padding: 24px;">
                        ...
                        <div style="background: #ffebee; border: 2px dashed #f44336; padding: 16px; border-radius: 8px; margin-bottom: 20px;">
                            <div style="font-size: 13px; color: #c62828; line-height: 1.5;">
                                ${isModalIssue ? 
                                    '⚠️ Sepertinya Anda belum mencatat Modal Awal hari ini. Gunakan tombol "Modal Awal" untuk mencatat modal dengan benar.' : 
                        ...
                        <div class="modal-footer" style="display: flex; gap: 12px; justify-content: flex-end;">
                            ...
                            ` : ''}
                            ${isModalIssue ? `
                            <button class="btn btn-primary" onclick="cashModule.closeModal('repairModal'); cashModule.openModalAwal();" style="padding: 12px 24px; border-radius: 10px; border: none; background: #ffc107; color: #333; font-weight: 600; cursor: pointer; font-size: 15px;">💰 Input Modal Awal</button>
                            ` : ''}
                        </div>
                    </div>
                </div>
    ...
    manualRepair(selisih) {
        if (!confirm(`Tambahkan Rp ${utils.formatNumber(selisih)} sebagai transaksi kas masuk?\n\nIni akan mencatat transaksi yang sebelumnya hilang.`)) {
            return;
        }
        ...
        dataManager.data.cashTransactions.push({
            ...
            amount: selisih,
            category: 'lainnya',
            note: 'Koreksi Kas (Manual Repair)',
            autoGenerated: true,
            repaired: true
        });
        ...
        this.renderHTML();
        this.renderTransactions();
        app.showToast(`✅ Data berhasil diperbaiki! Rp ${utils.formatNumber(selisih)} tercatat.`);
    },
    ...
    renderHTML() {
        ...
        // ✅ PERBAIKAN: Cek apakah perlu repair dengan logic yang lebih baik
        const currentCash = parseInt(dataManager.data.settings?.currentCash) || 0;
        const calculatedCash = this.calculateCashFromTransactions();
        const selisih = currentCash - calculatedCash;
        
        // Hanya tampilkan warning jika selisih signifikan dan bukan karena modal belum di-input
        const today = new Date();
        today.setHours(0,0,0,0);
        ...
        // Tampilkan warning hanya jika selisih > 0 dan bukan karena modal belum di-input
        const needsRepair = selisih > 0 && !(selisih === currentCash && !hasModalToday);
    ...
    // ✅ PERBAIKAN: saveModalAwal - hanya tambah transaksi sekali
    saveModalAwal() {
        ...
        // ✅ PERBAIKAN: Simpan modal ke settings.modalAwal untuk digunakan saat day change
        dataManager.data.settings.modalAwal = amount;
        
        // ✅ PERBAIKAN: Tambah ke currentCash (tambah, bukan replace)
        dataManager.data.settings.currentCash = currentCash + amount;
        
        // ✅ PERBAIKAN: Catat transaksi modal hanya sekali di sini
        dataManager.data.cashTransactions.push({
            id: Date.now(),
            date: new Date().toISOString(),
            type: 'modal_in',
